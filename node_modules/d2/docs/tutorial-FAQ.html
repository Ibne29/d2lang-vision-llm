...
<article>
    <h1>How do i? (FAQ)</h1>
<h2>Figure out which models i can use?</h2>
<p>The types of models to use include almost all the available endpoint points in DHIS2. The ones available are the <em>generic</em> endpoints.
...
<h2>How do i add translation sources and strings and get rid of <code>** name **</code></h2>
<p>Before calling the <code>init</code> we might want to register some sources for translations. The translation sources could be either specified by adding a file source
(in the form of a <code>.properties</code> file) or by adding individual strings.</p>
<h3>Adding file sources</h3>
<p>To add a file source you simply add it to the set of files to load by using the <code>i18n.sources.add</code> method.</p>
...
<h3>Adding individual strings</h3>
<p>To add strings/keys that you would like to get translated by the dhis2 api (These are the translation keys that you do not specify in your <code>.properties</code> files)
