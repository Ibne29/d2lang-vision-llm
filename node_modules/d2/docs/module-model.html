...
<h3>ModelDefinitions</h3>
<p>A <a href="module-model.ModelDefinition.html">module:model.ModelDefinition</a> is where it all starts. In d2 the schemas you have loaded are represented by a <code>ModelDefinition</code>.
...
<p>When working with the d2 object the <code>d2.models</code> object contains modelDefinitions for the schemas that you have loaded.
...
<p>The ModelDefinition allows you to load objects from the api. You can either load individual items or items in bulk.</p>
...
<p>or to load a list of objects (by default paging is enabled and it will only load the first 50 entries).</p>
...
<p>The ModelDefinition also allows you to filter on properties using a programatic syntax. To find out which
...
<p>Applying a filter like this will create a clone of the modelDefinition with the filter applied, so you can
...
<p>As you might have noticed we passed <code>{ paging: false }</code> to the <code>.list</code> method. Any options passed to the list method
...
<h2>ModelCollection</h2>
<p>A <a href="module-model.ModelCollection.html">ModelCollection</a> is simple a collection of Model objects. The ModelCollection is
...
<p>The ModelCollection does not only contain the Models, it also gives you access to a <a href="module-model.Pager.html">Pager</a>.
